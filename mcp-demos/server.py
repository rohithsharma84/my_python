...
tavily = TavilyClient(api_key=os.getenv("TAVILY_API_KEY"))  # Tavily SDK client :contentReference[oaicite:4]{index=4}
mcp = FastMCP(name="MarketIntel")              # FastMCP server scaffold :contentReference[oaicite:5]{index=5}
...
@mcp.tool(annotations={"title": "Company Overview"}, description="Provides a concise company profile with founding, headquarters, products, business model, and recent news.")
...
@mcp.tool(annotations={"title": "List Competitors"}, description="Finds the main direct and emerging competitors for a company, optionally filtered by category and region.")
...
@mcp.tool(annotations={"title": "Product Portfolio Map"}, description="Maps a company's product portfolio, including suites, tiers, and segment-focused offerings.")
...
@mcp.tool(annotations={"title": "Pricing Snapshot"}, description="Collects pricing information for a company or product, including tiers, billing cycles, discounts, and fees.")
...
@mcp.tool(annotations={"title": "Recent News Pulse"}, description="Summarizes recent company news such as funding, acquisitions, launches, and leadership changes.")
...
def main():
    ...
    mcp.run(transport="sse")  # or "stdio" if you prefer
